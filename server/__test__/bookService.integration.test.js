const testDb = require('../config/db.test'); // Import the test DB configuration
...
describe('bookService Integration Tests', () => {
  ...
  //   const books = await getBooks();
  //   expect(books.length).toBe(1);
  //   expect(books[0]).toMatchObject({ title: 'Default Book', author: 'Default Author', read: false, stripePriceId: 'default_price_id' });
  // });
  ...
  //   const book = await Book.create({ title: 'Old Book', author: 'Old Author', read: false, stripePriceId: 'price_123' });
  ...
  //   const book = await Book.create({ title: 'Book', author: 'Author', read: false, stripePriceId: 'price_123' });
